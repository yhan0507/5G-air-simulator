...
void PROPOSEDDownlinkPacketScheduler::DoSchedule()
{
    ...
    cout << "Start PROPOSED packet scheduler for node "
...
double PROPOSEDDownlinkPacketScheduler::ComputeSchedulingMetric(RadioBearer *bearer, double spectralEfficiency, int subChannel)
{
    /* Turn into your own algorithem
    //
    //
    //
    //
...
void PROPOSEDDownlinkPacketScheduler::Checkfairness()
{
    //According to Jain Fairness index

    unsigned long long numerator = 0;
    unsigned long long denominator = 0;

    for (std::map<int, int>::iterator it = TransmittedThroughputs.begin(); it != TransmittedThroughputs.end(); it++)
    {
        int temp = it->second;
        numerator += temp;
        denominator += pow(temp, 2);
    }
    if (numerator != 0 && denominator != 0)
    {
        unsigned long long temp_ = 0;
        fairness_index = pow(numerator, 2) / (TransmittedThroughputs.size() * denominator);
        DEBUG_LOG_START_1(SIM_ENV_SCHEDULER_DEBUG)
        temp_ = pow(numerator, 2);
        std::cout << "\tFairness_index: " << fairness_index << " Total_Throughput: " << numerator << std::endl;
        std::cout << "\tnumerator: " << temp_ << "/ Size: " << TransmittedThroughputs.size() << " numerator: " << temp_ << " denominator: " << denominator << std::endl;
        DEBUG_LOG_END
    }
}
...
{
    DEBUG_LOG_START_1(SIM_ENV_SCHEDULER_DEBUG)
    cout << " ---- PROPOSED DownlinkPacketScheduler::RBsAllocation\n";
    DEBUG_LOG_END
    ...
    if (_harq_active_)
    {
        ...
        for (auto harqFlow : flowsForRetransmission)
        {
            ...
            sort(schedulingMetricsIndices.begin(),
                 ...
                 [&localSchedulingMetrics](int i1, int i2)
                 { return localSchedulingMetrics[i1] < localSchedulingMetrics[i2]; });
    ...
    for (int rb = 0; rb < nbOfRBs; rb++)
    {
        ...
        for (int i = allocationsPerRB.at(rb); i < m_maxUsersPerRB; i++)
        {
            ...
            for (int k = 0; k < (int)flows->size(); k++)
            {
                if (metrics[rb][k] > targetMetric && metrics[rb][k] > -INFINITY && !l_bFlowScheduled[k] && flows->at(k)->IsHarqRetransmission() == false && find(HarqScheduledUEs.begin(), HarqScheduledUEs.end(), flows->at(k)->GetUe()->GetIDNetworkNode()) == HarqScheduledUEs.end() && find(scheduledFlows.begin(), scheduledFlows.end(), flows->at(k)) == scheduledFlows.end() && flows->at(k)->GetDataToTransmit() > 0)
                {
